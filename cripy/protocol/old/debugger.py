...
class BreakpointResolvedEvent(BaseEvent):
    ...
    def __init__(self,
        ...
        elif isinstance(breakpointId, list):
            breakpointId = [BreakpointId(**item) for item in breakpointId]
        self.breakpointId = breakpointId
        if isinstance(location, dict):
            location = Location(**location)
        elif isinstance(location, list):
            location = [Location(**item) for item in location]
        self.location = location
...
class PausedEvent(BaseEvent):
    ...
    def __init__(self,
        ...
        elif isinstance(callFrames, list):
            callFrames = [CallFrame(**item) for item in callFrames]
        ...
        elif isinstance(hitBreakpoints, list):
            hitBreakpoints = [(**item) for item in hitBreakpoints]
        self.hitBreakpoints = hitBreakpoints
        if isinstance(asyncStackTrace, dict):
            asyncStackTrace = Runtime.StackTrace(**asyncStackTrace)
        elif isinstance(asyncStackTrace, list):
            asyncStackTrace = [Runtime.StackTrace(**item) for item in asyncStackTrace]
        ...
        elif isinstance(asyncStackTraceId, list):
            asyncStackTraceId = [Runtime.StackTraceId(**item) for item in asyncStackTraceId]
        ...
        elif isinstance(asyncCallStackTraceId, list):
            asyncCallStackTraceId = [Runtime.StackTraceId(**item) for item in asyncCallStackTraceId]
...
class ScriptFailedToParseEvent(BaseEvent):
    ...
    def __init__(self,
        ...
        elif isinstance(scriptId, list):
            scriptId = [Runtime.ScriptId(**item) for item in scriptId]
        self.scriptId = scriptId
        ...
        elif isinstance(startLine, list):
            startLine = [int(**item) for item in startLine]
        self.startLine = startLine
        if isinstance(startColumn, dict):
            startColumn = int(**startColumn)
        elif isinstance(startColumn, list):
            startColumn = [int(**item) for item in startColumn]
        self.startColumn = startColumn
        if isinstance(endLine, dict):
            endLine = int(**endLine)
        elif isinstance(endLine, list):
            endLine = [int(**item) for item in endLine]
        self.endLine = endLine
        if isinstance(endColumn, dict):
            endColumn = int(**endColumn)
        elif isinstance(endColumn, list):
            endColumn = [int(**item) for item in endColumn]
        ...
        if isinstance(executionContextId, dict):
            ...
            executionContextId = [Runtime.ExecutionContextId(**item) for item in executionContextId]
        self.executionContextId = executionContextId
        if isinstance(hash, dict):
            hash = str(**hash)
        elif isinstance(hash, list):
            hash = [str(**item) for item in hash]
        self.hash = hash
        if isinstance(executionContextAuxData, dict):
            executionContextAuxData = dict(**executionContextAuxData)
        elif isinstance(executionContextAuxData, list):
            executionContextAuxData = [dict(**item) for item in executionContextAuxData]
        self.executionContextAuxData = executionContextAuxData
        if isinstance(sourceMapURL, dict):
            sourceMapURL = str(**sourceMapURL)
        elif isinstance(sourceMapURL, list):
            sourceMapURL = [str(**item) for item in sourceMapURL]
        self.sourceMapURL = sourceMapURL
        if isinstance(hasSourceURL, dict):
            hasSourceURL = bool(**hasSourceURL)
        elif isinstance(hasSourceURL, list):
            hasSourceURL = [bool(**item) for item in hasSourceURL]
        self.hasSourceURL = hasSourceURL
        if isinstance(isModule, dict):
            isModule = bool(**isModule)
        elif isinstance(isModule, list):
            isModule = [bool(**item) for item in isModule]
        self.isModule = isModule
        if isinstance(length, dict):
            length = int(**length)
        elif isinstance(length, list):
            length = [int(**item) for item in length]
        self.length = length
        if isinstance(stackTrace, dict):
            stackTrace = Runtime.StackTrace(**stackTrace)
        elif isinstance(stackTrace, list):
            stackTrace = [Runtime.StackTrace(**item) for item in stackTrace]
...
class ScriptParsedEvent(BaseEvent):
    ...
    def __init__(self,
        ...
        elif isinstance(isLiveEdit, list):
            isLiveEdit = [bool(**item) for item in isLiveEdit]
