        ...
        def convert(result: dict):
            ...
            for name, val in result.items():
                ...
                if issubclass(expected_type_, ChromeTypeBase):
                    ...
                    raise ValueError(
                        "%s is not expected type %s, instead is %s"
                        % (val, expected_type_, val)
                    )
...
class BaseEvent:
    ...
    def hash_(self):
        ...
        serialized_id_params = ",".join(
            ["=".join([p, str(v)]) for p, v in hashable_params.items()]
        )
...
def json_to_event(payload) -> Optional[BaseEvent]:
    try:
        ...
        log.error(
            'invalid method name "%s", must contain a module and event joined with a "."'
            % payload["method"]
        )
        return None
    module_name = "cripy.protocol.%s" % prot_name.lower()
    try:
...
class CRIPYJSONEncoder(json.JSONEncoder):
