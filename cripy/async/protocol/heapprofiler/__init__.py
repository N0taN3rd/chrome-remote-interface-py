...
from cripy.async.protocol.heapprofiler import events as Events
from cripy.async.protocol.heapprofiler import types as Types
...
class HeapProfiler(object):
    ...
    async def addInspectedHeapObject(self, heapObjectId: str) -> Optional[dict]:
        ...
        mayberes = await self.chrome.send(
            "HeapProfiler.addInspectedHeapObject", msg_dict
        )
        return mayberes
    ...
    async def getObjectByHeapObjectId(
        ...
        mayberes = await self.chrome.send(
            "HeapProfiler.getObjectByHeapObjectId", msg_dict
        )
        res = await mayberes
    ...
    async def startTrackingHeapObjects(
        ...
        mayberes = await self.chrome.send(
            "HeapProfiler.startTrackingHeapObjects", msg_dict
        )
        return mayberes
    ...
    async def stopTrackingHeapObjects(
        ...
        mayberes = await self.chrome.send(
            "HeapProfiler.stopTrackingHeapObjects", msg_dict
        )
        return mayberes
