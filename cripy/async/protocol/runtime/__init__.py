...
from cripy.async.protocol.runtime import events as Events
from cripy.async.protocol.runtime import types as Types
...
other objects in their object group.
    ...
    async def setMaxCallStackSizeToCapture(self, size: int) -> Optional[dict]:
        ...
        if size is not None:
            msg_dict["size"] = size
        mayberes = await self.chrome.send(
            "Runtime.setMaxCallStackSizeToCapture", msg_dict
        )
        return mayberes
