...
from cripy.chrome.connection import Connection, TargetConnection
...
class Chrome(EventEmitter):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self._connection.set_close_callback(
            lambda: self.emit(Chrome.Events.Disconnected)
        )
    ...
    async def create(
        ...
        **kwargs: Any
        ...
        options = merge_dict(options, kwargs)
        browser = Chrome(connection, options, process, closeCallback)
    ...
    async def newPage(self) -> Page:
        ...
        targetId = (
            await self._connection.send("Target.createTarget", {"url": "about:blank"})
        ).get("targetId")
...
class Target(object):
    ...
    def __init__(self, browser: Chrome, targetInfo: Dict) -> None:
    ...
    async def createCDPSession(self) -> TargetConnection:
        """Create a Chrome Devtools Protocol session attached to the target."""
        return await self._browser._connection.create_session(self._targetId)

    async def page(self) -> Optional[Page]:
        """Get page of this target."""
        if self._targetInfo["type"] == "page" and self._page is None:
            client = await self._browser._connection.create_session(self._targetId)
