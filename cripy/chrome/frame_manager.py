...
from cripy.chrome.element_handle import ElementHandle
from cripy.chrome.errors import NetworkError
from cripy.chrome.execution_context import ExecutionContext, JSHandle
from cripy.chrome.errors import ElementHandleError, PageError, TimeoutError
...
"""
    ...
    async def addScriptTag(self, options: Dict) -> ElementHandle:
        ...
        if isinstance(options.get("url"), str):
            ...
            try:
                return (
                    await context.evaluateHandle(addScriptUrl, url)  # type: ignore
                ).asElement()
        ...
        if isinstance(options.get("path"), str):
            ...
            return (
                await context.evaluateHandle(addScriptContent, contents)  # type: ignore
            ).asElement()
    ...
    async def addStyleTag(self, options: Dict) -> ElementHandle:
        ...
        if isinstance(options.get("url"), str):
            ...
            try:
                return (
                    await context.evaluateHandle(addStyleUrl, url)  # type: ignore
                ).asElement()
            except ElementHandleError as e:
        ...
        if isinstance(options.get("path"), str):
            ...
            return (
                await context.evaluateHandle(addStyleContent, contents)  # type: ignore
            ).asElement()
