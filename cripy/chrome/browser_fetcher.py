...
class BrowserFetcher(object):
    ...
    def __init__(self):
        self.downloads_folder = Path.home() / ".cripy" / "local-chromium"
        self.default_download_host = "https://storage.googleapis.com"
        self.download_host = os.environ.get(
            "CRIPY_DOWNLOAD_HOST", self.default_download_host
        )
        self.base_url = f"{self.download_host}/chromium-browser-snapshots"
        self.downloadURLs = None
        self.chromiumExecutable = None
        self.revision = DEFAULT_REVISION
        self.update_download_revision()

    def update_download_revision(self, revision=DEFAULT_REVISION):
