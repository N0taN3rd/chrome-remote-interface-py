...
class ConsoleAPICalledEvent(BaseEvent):
    ...
    def __init__(
        ...
        if isinstance(type, dict):
            ...
            type = [str(**item) for item in type]
        self.type = type
        if isinstance(args, dict):
            args = [RemoteObject](**args)
        elif isinstance(args, list):
            args = [RemoteObject(**item) for item in args]
        self.args = args
        ...
        if isinstance(timestamp, dict):
            ...
            timestamp = [Timestamp(**item) for item in timestamp]
        self.timestamp = timestamp
        if isinstance(stackTrace, dict):
            stackTrace = StackTrace(**stackTrace)
        elif isinstance(stackTrace, list):
            stackTrace = [StackTrace(**item) for item in stackTrace]
        self.stackTrace = stackTrace
...
class ExceptionRevokedEvent(BaseEvent):
    ...
    def __init__(self, reason: Union["str", dict], exceptionId: Union["int", dict]):
        ...
        elif isinstance(exceptionId, list):
            exceptionId = [int(**item) for item in exceptionId]
        self.exceptionId = exceptionId
...
class ExceptionThrownEvent(BaseEvent):
    ...
    def __init__(
        ...
        elif isinstance(exceptionDetails, list):
            exceptionDetails = [ExceptionDetails(**item) for item in exceptionDetails]
...
class ExecutionContextCreatedEvent(BaseEvent):
    ...
    def __init__(self, context: Union["ExecutionContextDescription", dict]):
        if isinstance(context, dict):
            ...
            context = [ExecutionContextDescription(**item) for item in context]
        self.context = context
...
class InspectRequestedEvent(BaseEvent):
    ...
    def __init__(self, object: Union["RemoteObject", dict], hints: Union["dict", dict]):
        if isinstance(object, dict):
            ...
            object = [RemoteObject(**item) for item in object]
        self.object = object
